...
def _apply_feature_intervention(
    ...
    attribution/intervention mismatch documented at the top of this file.
    Passing a `(batch,)` tensor of positions (from `utils.last_real_token_index`)
    restricts the edit to just that one token per row, so the intervention
    targets the same position the attribution actually describes.
...
def get_model_intervention_effects(
    ...
    mode="steering": encode → a_i += alpha_i (signed, additive) → decode.

    `intervene_scope="all"` (default) edits every token position, matching the
    historical behaviour documented at the top of this file. `"last"` edits only
    the last real token per row (`utils.last_real_token_index`), matching the
    token position the attribution was actually computed at and removing the
    attribution/intervention position mismatch — at the cost of a much smaller
    forward-pass perturbation, so effect sizes from the two scopes are not
    comparable.
