...
as of this writing it is given; anything else is marked `[TODO: pull from
run2]`.
...
- **General definition**: `IG_j = (x_j − baseline_j) · ∫₀¹ ∂f/∂x_j
  (baseline + α(x−baseline)) dα`, approximated (MLP arm) with a 50-step
...
- **MLP arm has no `probe` curve**: the MLP's `probe_saliency`
  ...
  not missing data.
- **Sweep**: `k ∈ {5, 10, 20, 30, 50, 65}` × `α ∈ {0.5, 1, 2, 5, 10, 20,
  50}` (from `steering_config.json`), giving 7 vectors (4 logistic + 3
...
1. **Attribution is last-token; intervention is all-token.** Attributions
   describe the SAE vector at one sequence position; `_apply_feature_
   intervention` edits every token position. Part of any faithfulness gap
